# Relationships in the online era: An investigation of social media usage patterns by relationship status {.unnumbered}
...
fig1 <- media%>%
  ...
  ggplot(aes(x = Time, y = n,fill = MStat, text = paste0("Time: ", Time, "<br>Marital Status: ", MStat, "<br>Percentage: ", round(Percentage, 1), "%"))) +
  geom_bar(stat = "identity", alpha=0.85, position='fill')+
  labs(x='Time Spent on Social Media', y='Propotion of Use by Marital Status', title='Fig. 1: Time on Social Media by Marital Status', fill = 'Marital Status', caption = 'Plot created by Tanisha Dodla')+
  labs(x='Time Spent on Social Media', y='Propotion of Use by Marital Status', MStat = 'Marital Status', title='Time on Social Media by Marital Status', caption = 'Plot created by Tanisha Dodla')+
  theme_minimal()+
...
fig2 <- media%>%
  ...
  count(Gender) %>%
  mutate(Percentage = n / sum(n) * 100) %>%
  ggplot(aes(x = Time,y = n, fill = Gender, text = paste0("Time Spent: ", Time,"<br>Gender: ", Gender,"<br>Percentage: ", round(Percentage, 1), "%"))) +
  geom_bar(stat = "identity", position='fill' , alpha=0.85)+
  labs(x='Time spent during the day on Social Media', y='Propotion of Use by Gender', title='Fig. 2: Time on Social Media by Gender', caption = 'Plot created by Tanisha Dodla')+
  theme_minimal()+
...
fig3 <- media %>% 
  ...
  ggplot(aes( x = Platform, fill = Gender, y = count,text = paste0("Platform: ", Platform,"<br>Gender: ", Gender,"<br>Percentage: ", round(Percentage, 1), "%") )) +
  geom_bar(stat = "identity", position='fill' , alpha=0.85)+
  labs(x='Social Media Platform', y='Propotion of Use by Gender', title='Fig. 3: Use of Social Media Platform by Gender', caption = 'Use of social media by useage of multiple apps. Plot created by Tanisha Dodla')+
  labs(x='Social Media Platform', y='Propotion of Use by Gender', title='Use of Social Media Platform by Gender', caption = 'Use of social media by useage of multiple apps')+
  theme_minimal()+
  scale_fill_manual(values = c("Female" = "#B0476E", "Male" = "#C9732A")) +
  theme(text = element_text(hjust = 0.5, size=12, family='serif'), plot.title=element_text(hjust = 0.5))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
