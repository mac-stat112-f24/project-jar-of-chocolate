...
fig1 <- media%>%
  ...
  ggplot(aes(y = Time, x = n,fill = MStat, text = paste0("Time: ", Time, "<br>Marital Status: ", MStat, "<br>Percentage: ", round(Percentage, 1), "%"))) +
  ...
  labs(y='Time Spent on Social Media', x='Propotion of Use by Marital Status', MStat = 'Marital Status', title='Fig 1: Time on Social Media by Marital Status')+
  theme_minimal()+
...
fig2 <- media%>%
  ...
  count(Gender) %>%
  mutate(Percentage = n / sum(n) * 100) %>%
  ggplot(aes(y = Time,x = n, fill = Gender, text = paste0("Time Spent: ", Time,"<br>Gender: ", Gender,"<br>Percentage: ", round(Percentage, 1), "%"))) +
  geom_bar(stat = "identity", position='fill' , alpha=0.85)+
  labs(y='Time spent during the day on Social Media', x='Propotion of Use by Gender', title='Fig. 2: Time on Social Media by Gender')+
  theme_minimal()+
